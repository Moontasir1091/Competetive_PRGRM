...
void String_generate(string s) {
    if(s.empty())return;  // If the string is empty, return
    if(unique_strings.count(s)){ // if the string is already in the set, return
        return;
    }

    unique_strings.insert(s);

    String_generate(s.substr(1));

    if(s.size() >= 2){
        string temp = s[0] + s.substr(2);
        String_generate(temp);
...
int main() {
    ...
    while(t--) {
        ...
        unique_strings.clear(); // reset for each test case
